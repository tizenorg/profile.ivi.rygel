/* rygel-media-export-jpeg-writer.c generated by valac 0.18.0, the Vala compiler
...
RygelMediaExportJPEGWriter* rygel_media_export_jpeg_writer_construct (GType object_type, GError** error) {
	...
	_tmp0_ = gst_parse_launch_full ("appsrc name=src ! decodebin ! " "autovideoconvert ! " "jpegenc ! giosink name=sink", NULL, GST_PARSE_FLAG_NONE, &_inner_error_);
	_tmp1_ = _tmp0_;
	...
	self->priv->bin = G_TYPE_CHECK_INSTANCE_TYPE (_tmp1_, gst_bin_get_type ()) ? ((GstBin*) _tmp1_) : NULL;
	_tmp2_ = self->priv->bin;
	_tmp3_ = gst_bin_get_by_name (_tmp2_, "src");
	_g_object_unref0 (self->priv->appsrc);
	self->priv->appsrc = G_TYPE_CHECK_INSTANCE_TYPE (_tmp3_, gst_app_src_get_type ()) ? ((GstAppSrc*) _tmp3_) : NULL;
	...
	bus = _tmp7_;
	_tmp8_ = bus;
	gst_bus_add_signal_watch_full (_tmp8_, G_PRIORITY_DEFAULT);
	_tmp9_ = bus;
