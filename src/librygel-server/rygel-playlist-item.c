/* rygel-playlist-item.c generated by valac 0.18.0, the Vala compiler
...
struct _RygelPlaylistItemPrivate {
	gchar* _creator;
};
...
#define RYGEL_PLAYLIST_ITEM_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), RYGEL_TYPE_PLAYLIST_ITEM, RygelPlaylistItemPrivate))
enum  {
	RYGEL_PLAYLIST_ITEM_DUMMY_PROPERTY,
	RYGEL_PLAYLIST_ITEM_CREATOR
};
...
const gchar* rygel_playlist_item_get_creator (RygelPlaylistItem* self);
void rygel_playlist_item_set_creator (RygelPlaylistItem* self, const gchar* value);
static void rygel_playlist_item_finalize (GObject* obj);
static void _vala_rygel_playlist_item_get_property (GObject * object, guint property_id, GValue * value, GParamSpec * pspec);
static void _vala_rygel_playlist_item_set_property (GObject * object, guint property_id, const GValue * value, GParamSpec * pspec);
...
static void rygel_playlist_item_class_init (RygelPlaylistItemClass * klass) {
	...
	g_type_class_add_private (klass, sizeof (RygelPlaylistItemPrivate));
	...
	G_OBJECT_CLASS (klass)->get_property = _vala_rygel_playlist_item_get_property;
	G_OBJECT_CLASS (klass)->set_property = _vala_rygel_playlist_item_set_property;
	G_OBJECT_CLASS (klass)->finalize = rygel_playlist_item_finalize;
	g_object_class_install_property (G_OBJECT_CLASS (klass), RYGEL_PLAYLIST_ITEM_CREATOR, g_param_spec_string ("creator", "creator", "creator", NULL, G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB | G_PARAM_READABLE | G_PARAM_WRITABLE));
}


static void rygel_playlist_item_instance_init (RygelPlaylistItem * self) {
	self->priv = RYGEL_PLAYLIST_ITEM_GET_PRIVATE (self);
}


static void rygel_playlist_item_finalize (GObject* obj) {
	RygelPlaylistItem * self;
	self = G_TYPE_CHECK_INSTANCE_CAST (obj, RYGEL_TYPE_PLAYLIST_ITEM, RygelPlaylistItem);
	_g_free0 (self->priv->_creator);
	G_OBJECT_CLASS (rygel_playlist_item_parent_class)->finalize (obj);
}
...
static void _vala_rygel_playlist_item_get_property (GObject * object, guint property_id, GValue * value, GParamSpec * pspec) {
	RygelPlaylistItem * self;
	self = G_TYPE_CHECK_INSTANCE_CAST (object, RYGEL_TYPE_PLAYLIST_ITEM, RygelPlaylistItem);
	switch (property_id) {
		case RYGEL_PLAYLIST_ITEM_CREATOR:
		g_value_set_string (value, rygel_playlist_item_get_creator (self));
		break;
		default:
...
static void _vala_rygel_playlist_item_set_property (GObject * object, guint property_id, const GValue * value, GParamSpec * pspec) {
	...
	switch (property_id) {
		case RYGEL_PLAYLIST_ITEM_CREATOR:
		rygel_playlist_item_set_creator (self, g_value_get_string (value));
		break;
		default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
		break;
