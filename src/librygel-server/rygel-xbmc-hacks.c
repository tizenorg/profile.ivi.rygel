/* rygel-xbmc-hacks.c generated by valac 0.18.0, the Vala compiler
...
static void rygel_xbmc_hacks_real_apply (RygelClientHacks* base, RygelMediaItem* item) {
	...
	_tmp2_ = item;
	_tmp3_ = rygel_media_item_get_mime_type (_tmp2_);
	_tmp4_ = _tmp3_;
	if (g_strcmp0 (_tmp4_, "audio/mp4") == 0) {
		_tmp1_ = TRUE;
	} else {
		...
		_tmp7_ = _tmp6_;
		_tmp1_ = g_strcmp0 (_tmp7_, "audio/3gpp") == 0;
	}
	_tmp8_ = _tmp1_;
	if (_tmp8_) {
		_tmp0_ = TRUE;
		...
		const gchar* _tmp11_;
		_tmp9_ = item;
		_tmp10_ = rygel_media_item_get_mime_type (_tmp9_);
		_tmp11_ = _tmp10_;
		_tmp0_ = g_strcmp0 (_tmp11_, "audio/vnd.dlna.adts") == 0;
	}
	_tmp12_ = _tmp0_;
	if (_tmp12_) {
		RygelMediaItem* _tmp13_;
		_tmp13_ = item;
		rygel_media_item_set_mime_type (_tmp13_, "audio/aac");
	}
}
